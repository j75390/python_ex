...
'''
한빛 마트는 고객 감사의 일환으로 '오늘의 할인' 이벤트를 진행할 계획입니다.
...
def getDiscountPrice(rate):
    ...
    for goodsName in standerdPrice.keys():
        disPrice = int(standerdPrice[goodsName] * (1 - (rate / 100)))
...
def printPrice(priceList):
    for goodsName,goodsPrice in priceList.items():
        print(f'{goodsName}\t: {standerdPrice[goodsName]}원 ---> {inputRatData}%DC({goodsPrice})원')

inputRatData = int(input('오늘의 할인율 입력: '))

discountPrice = getDiscountPrice(inputRatData)
