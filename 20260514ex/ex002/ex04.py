...
valuse = student.values()
print(f'values: {valuse}')   # dict_values([123456789, '홍길자', '20', 'M'])
print(f'values type: {type(valuse)}')

for value in valuse:
    print(f'value: {value}')

items = student.items()     # key $ value
print(f'items: {items}')    # dict_item([('학번', 123456789), ('이름', '홍길자'), ('나이', '20'), ('성별', 'M')])
for item in items:
    print(f'item: {item}')
    print(f'item, item[1]: {item[0]}, {item[1]}')

'''
item 튜플 ==> ('학번', 123456789) ==> item[0], itme[1]
'''
